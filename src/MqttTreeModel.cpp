...
QVariant MqttTreeModel::newCustomType(const QString &text, int position)
{
...
void MqttTreeModel::addTopic(const QString &topic, const QString &message)
{
    ...
    int position = 0;
    columnData << newCustomType(topic, position) << newCustomType(message, position);
...
{
//    if(m_mqttRootTreeItem->childCount()==0){
//        return;
//    }
